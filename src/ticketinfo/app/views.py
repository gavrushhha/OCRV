...
from .models import Routes, Goods_and_services, \
    Passengers, Purch_tickets, Purch_goods_services
...
def viewRoutes(request: WSGIRequest) -> HttpResponse:
    """
    Функция отображает запрос на поиск поездок по определенному городу на ./viewRoutes.html
    """
...
def profile(request: WSGIRequest) -> HttpResponse:
    ...
    try:
        ...
        Passengers.objects.filter(id=pas_id).update(age=age, sex=sex, marit_status=marit_status, education=education, childs=childs)
